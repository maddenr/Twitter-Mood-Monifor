...
#
	def __init__(self,geoString):
		...
		self.HappyRegex = re.compile("\s[;:=B8X]{1}[-]?[)}\]*DP]{1}(?:\s|\Z)|\s[(c{[*]{1}[-]?[;:=B8X]{1}(?:\s|\Z)", re.IGNORECASE)
...
#Out:	None
	def gatherTweetData(self):
		...
		for status in json.loads(searchResults)["statuses"]:
			#determine highest id
			# use since id to ensure unique results
